		...
		if (strcmp(child.name(), "RenderTarget") == 0) {
			...
			if (strcmp(targetName, "FX_BACKBUFFER") == 0) {
				...
				if (fbo) {
					...
					for (int i = 0; i < fbo->GetNumResources(); i++) {
						std::shared_ptr<ResourceGL> renderTex(new ResourceGL(*std::static_pointer_cast<ResourceGL>(fbo->GetTextureResource(i))));
						SaveObject(renderTex); //instead of SaveObject, use the repository
						fboCopy->AddTextureResource(renderTex);
						//pass->CreateData(renderTex); //DO NOT ADD TO PASS CUZ IT DOESNT NEED BINDING
						//renderTex->SetActive(false); //you dont want it to execute here...
					}
					std::shared_ptr<ResourceGL> depthTex(new ResourceGL(*std::static_pointer_cast<ResourceGL>(fbo->GetDSTTexture())));
					if (depthTex) {
						SaveObject(depthTex);
						fboCopy->SetDSTTexture(depthTex);
						//pass->CreateData(depthTex);
	...
	for (pugi::xml_node child = node.first_child(); child; child = child.next_sibling()) {
		if (strcmp(child.name(), "RenderTexture") == 0) {
			...
			if (res) {
				...
				fbo->AddTextureResource(resCopy);
			}
			continue;
		}
...
bool flFxParserGL::HandleStateNode(pugi::xml_node node) {
	...
	std::shared_ptr<StateGroupGL> stateGroup = std::static_pointer_cast<StateGroupGL>(m_CurrPass->Find(FX_STATE_GROUP_DESC_FLAG));
