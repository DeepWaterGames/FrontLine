...
bool PipelineGL::Validate() {
	...
	for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++) {
		...
		std::shared_ptr<GLSLShader> glslShader = std::static_pointer_cast<GLSLShader>(info->container.lock()->Find(info->id));
		//omg, its cuz shader isint validated yet
		info->container.lock()->ForceValidate(glslShader);
...
bool PipelineGL::Invalidate() {
	for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++)
	{
		PassDescInfo* info = *iter;
		std::shared_ptr<GLSLShader> glslShader = std::static_pointer_cast<GLSLShader>(info->container.lock()->Find(info->id));
		info->container.lock()->ForceInvalidate(glslShader);
	}
	glDeleteProgramPipelines(1, &m_Id);
	SetValidated(false);
	return true;
...
bool ShaderParameterGL::Validate() {
	...
	int p = 0;
	std::shared_ptr<PipelineGL> glslPipeline = std::static_pointer_cast<PipelineGL>(m_Parent.lock()->Find((fxState)FX_PIPELINE_DESC_FLAG));
	std::shared_ptr<GLSLShader> glslShader = std::static_pointer_cast<GLSLShader>(glslPipeline->FindData(p));
	while (glslShader) {
		Target t;
		glslShader->Bind();
		...
		glslShader->Unbind();
		p++;
		glslShader = std::static_pointer_cast<GLSLShader>(glslPipeline->FindData(p));
...
bool ShaderParameterGL::Update() {
	for (auto iter = m_Targets.begin(); iter != m_Targets.end(); iter++) {
		Target t = (*iter);
		std::shared_ptr<PipelineGL> glslPipeline = std::static_pointer_cast<PipelineGL>(m_Parent.lock()->Find((fxState)FX_PIPELINE_DESC_FLAG));
		std::shared_ptr<GLSLShader> glslShader = std::static_pointer_cast<GLSLShader>(glslPipeline->FindData(t.shaderNumber));
...
bool SamplerStateGL::UpdateTexture(int unit) {
	...
	if (m_HasChanged) {
		for (auto iter = m_States.begin(); iter != m_States.end(); iter++) {
			switch ((unsigned int)(*iter)->m_State) {
...
bool ResourceGL::Validate() {
	...
	switch ((unsigned int)m_ResourceType) {
...
bool FrameBufferObjectGL::Validate() {
	//dont need this anymore cuz all the resources are added to the pass already
	...
	{
		PassDescInfo* info = *iter;
		std::shared_ptr<ResourceGL> res = std::static_pointer_cast<ResourceGL>(info->container.lock()->Find(info->id));
		info->container.lock()->ForceValidate(res);
	}
	glGenFramebuffers(1, &m_Id);

	int size = GetDataSize(); //minus one cuz of the depth buffer
	if (size > 0) {
		...
		for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++)
		{
			PassDescInfo* info = *iter;
			std::shared_ptr<ResourceGL> res = std::static_pointer_cast<ResourceGL>(info->container.lock()->Find(info->id));
			if (res->GetResourceType() == FX_RESOURCE_RENDERTEX)
			{
				glBindTexture(GL_TEXTURE_2D, res->GetTextureId());
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + res->GetFBOAttachment(), GL_TEXTURE_2D, res->GetTextureId(), 0);
			}
		}
		//glBindTexture(GL_TEXTURE_2D, 0); 
		std::shared_ptr<ResourceGL> depthTex = NULL;
		for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++)
		{
			PassDescInfo* info = *iter;
			depthTex = std::static_pointer_cast<ResourceGL>(info->container.lock()->Find(info->id));
			if (depthTex->GetResourceType() == FX_RESOURCE_DEPTHTEX)
			{
				break;
			}
		}
		...
		{
			PassDescInfo* info = *iter;
			std::shared_ptr<ResourceGL> tex = std::static_pointer_cast<ResourceGL>(info->container.lock()->Find(info->id));
			if (tex->GetResourceType() == FX_RESOURCE_RENDERTEX)
			{
				renderTexSize++;
			}
		}
		GLenum* drawBuffers = new GLenum[renderTexSize];
		int count = 0;
		for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++)
		{
			PassDescInfo* info = *iter;
			...
			if (res->GetResourceType() == FX_RESOURCE_RENDERTEX)
			{
				drawBuffers[count] = GL_COLOR_ATTACHMENT0 + res->GetFBOAttachment();
				count++;
			}
		}
...
bool FrameBufferObjectGL::Invalidate() {
	for (auto iter = m_Data.begin(); iter != m_Data.end(); iter++)
	{
		PassDescInfo* info = *iter;
		std::shared_ptr<ResourceGL> res = std::static_pointer_cast<ResourceGL>(info->container.lock()->Find(info->id));
		info->container.lock()->ForceInvalidate(res);
	}
	glDeleteFramebuffers(1, &m_Id);
...
bool RenderingModeGL::Execute() {
	switch ((unsigned int)m_RenderingMode) {
