...
int main()
{
	...
	if(s1 == pipeline->FindData("testShader")) {
	...
	std::cout << "Before: " << demoPass2->Find((fxState)FX_DO_NOTHING)->GetName() << std::endl;
	std::shared_ptr<PassDesc> demoPassDesc2(new PassDesc(FX_DO_NOTHING, "demo2"));
	demoPass2->Replace("demo1", demoPassDesc2);
	std::cout << "After: " << demoPass2->Find((fxState)FX_DO_NOTHING)->GetName() << std::endl;
